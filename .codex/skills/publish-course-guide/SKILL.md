...
**Verify the tees played and every hole length against `src/lib/scorecard-data.js` before writing them into copy** (`tees[]` holds `totalLengthMeters` and `holeLengthsMeters` per tee colour). The course map poster he photographs gives a total with no tee colour, and tee names differ by course. At Son Quint the 6,021m is the white tees, which are the longest and hardest set there (white 6,021m, yellow 5,750m, blue 5,325m, red 4,987m in `tees[]`), so "white" and "back" are the same tee; the first draft labelled it "Tips" and gave the 12th as "around 200m" when the card says 195m. Do not assume white is a forward tee: read the lengths in `tees[]`. I also wrongly recorded on 2026-09-24 that his notes contradicted themselves ("white tees" vs "back tees") and then wrote "tougher than it looks even from the white tees" into the meta description, which read as if white were easy; Andy caught it from a WhatsApp link preview. If the notes and the scorecard disagree, ask; don't pick one silently. Also mark which facts came from site data rather than his round (e.g. "wide fairways", "four tee positions" came from the course-listing copy) so he can confirm he is happy owning them in first person.
...
Four registrations. None is handled by a script, but since 2026-09-26 the first drives most of the site and `npm run check:guide-surfaces` (part of `check:content`, so it also runs in pre-push and CI) fails if any of them is missing. The original misses (2026-09-09, 2026-09-26) all shipped green because nothing checked them.

1. **Add `reviewSlug: '{slug}'` to the course's own entry in `src/lib/golf-courses-data.js`. This is the single source of truth for "this course has a review".** Everything below derives from it through `getCourseReviewSlug(name)` in `src/lib/golf-courses-helpers.js`, so none of them is edited by hand: the green-fees tool (`/tools/green-fees`, all 7 locales, links to the locale's own review page), both course selectors (`tools/course-selector` and `course-selector`), the Chinese selector (`getZhGuideHref`), the carousel keyword sync (`syncGuideKeywordFacts`), the par check (`check-guide-review-keywords.js`) and the font test (`tests/font-consistency.spec.js`, which reads `REVIEW_POST_SLUGS`). If a course's name on one of those surfaces does not resolve (Roman numerals, a missing bracket, a missing cedilla), `check:guide-surfaces` fails naming the file and the name; add an alias to `REVIEW_LOOKUP_NAME_FIX` in the helper rather than typing a link on the surface. Do not add it before Andy has approved and the translations are live: the guard check rejects a `reviewSlug` whose slug is not in `REVIEW_POST_SLUGS`.
2. **Ask Andy where the card goes in the carousel (he expects to be asked; the default of "at the end" is not the answer).** Then add a card entry to `liveGuides` in `src/lib/guides-content.js` (en) at that position: `slug`, `badge`, `badgeGold: true` (all course reviews are gold, on both the review page and the carousel card, decided by Andy 2026-09-24 after four cards had been plain for no reason; the gold badge is only a colour style and does not signal rating), `img` (the Step 1 card image), `imgPosition`, `title`, `intro`, `readTime`, `keywords`. Then add a matching overlay to all 6 locale blocks in `src/lib/guides-content-localized.js` at the **same position** — same fields except `img`/`imgPosition`, which inherit from English. The English entry is 4-space indented and the localized ones 6-space; anchor the insertion on the `"slug": "<the card that should follow it>"` line, and assert the anchor count (1 for English, 6 for the localized file) before writing.
3. **Course Guides app (`mmg-tools`, separate repo, separate Netlify deploy, easy to forget).** Nothing is edited by hand any more: the app builds each card's "Read full review" link at runtime from `reviewSlug` in `mmg-tools/guide/course-sync.js`, which `node scripts/sync-pricing.js` generates from the website's `golf-courses-data.js`. So, once item 1 is deployed: in `mmg-tools` run `node --no-warnings scripts/sync-pricing.js`, confirm `git status` shows only `guide/course-sync.js` changed (it reads the pricing master, so an unrelated price change in the Drive master will show up too, which is fine but say so), bump the date suffix in `guide/sw.js`'s `VERSION` (PWA cache rule; add a letter if today's suffix is already used), then commit and push to `mmg-tools` `master`. The weekly Monday `pricing-publish` also runs this generator, so the link would appear eventually on its own; do it in the same session anyway. If `mmg-tools` has other sessions' uncommitted files, use an isolated `git worktree add <tmp> origin/master`, commit and `git push origin HEAD:master` from there, then remove the worktree, so their unfinished work is never staged (a worktree has no `node_modules`; run `node scripts/check-text-corruption.js` and `node scripts/check-js-parse.js` by hand and say you bypassed the hook and why). After Netlify deploys, `curl https://guide.mrmallorcagolf.com/sw.js` should show the new `VERSION`.
4. **Update the list of courses Andy has played** (Andy asks for this every time, so it is a required step, not an afterthought; do it in the same session as the go-live and tell him it is done) in Drive `MMG_ENCYCLOPAEDIA_DATA_MASTER.md`. Mechanics learned 2026-09-24: the file is **CRLF**, so read and write with `newline=""` and normalise before matching; the numbering is continuous across both sections (played 1..N, researched N+1..24), so when a course moves up only the researched entries *between* the old and new number shift (Son Quint 13 to 12 renumbered only Son Vida, 12 to 13); the two section headings ("## N Played Courses", "## N Researched Courses") and the Coverage Status counts drift out of step with the entries, so recount them from the entries rather than adding one; `PRACTICAL` lines are generated from the course master, so carry them across unchanged; add a dated "update" line under the title. The steps in detail:
...
## Step 6.5 — What updates itself, and what the guard check enforces

Found 2026-09-26 from a screenshot of `/tools/green-fees` showing Son Quint as "Coming soon" three days after its review went live: about ten surfaces each kept a hand-typed link to the review, and the two newest reviews (T Golf Palma, Son Quint) were missing from all of them. They are now derived, so **there is nothing to remember for them**. This step is what to expect, and the one command to run.

**Derived from `reviewSlug` (Step 6 item 1). Do not edit these per course:**
`GreenFeesClient.jsx` (link, locale-aware), `CourseSelectorToolClient.jsx`, `CourseSelectorClient.jsx`, `zh-course-editorial.js` (`getZhGuideHref`), `guides-content.js` keyword sync, `check-guide-review-keywords.js` (slug to course map), `tests/font-consistency.spec.js` (slugs from `REVIEW_POST_SLUGS`), and the Course Guides app's review links (via the generated `guide/course-sync.js`, Step 6 item 3). `COURSE_REVIEW_SLUGS` no longer exists. If you find a hand-typed `guideUrl`, `reviewSlug:'...'`, `guideSlug` or a `/guides/...-review` href in one of these files, that is a regression: delete it and derive.

**Still typed by hand, and enforced by `npm run check:guide-surfaces` (it fails naming the slug and the missing surface):**

| Surface | What to add |
|---|---|
| `public/llms.txt` | A `- [<Course> Review](https://www.mrmallorcagolf.com/guides/{slug}): <one factual line>` under the Guides list. Reuse facts from the review's approved `metadata.description`; the voice guide applies (no em dashes, banned words) |
| Carousel card, all 7 locales | Step 6 item 2 (`liveGuides` in `guides-content.js` plus the six overlays in `guides-content-localized.js`) |
| `mmg-tools/guide/course-sync.js` (generated) | Not typed: run the sync in Step 6 item 3. The check fails naming the slug if the generated file lacks its `reviewSlug` |
| `mmg-tools/pricing/edit/confirmed/consumer-map.json` | Add `mrmallorcagolf-real/src/lib/guide-post-content-localized/{slug}.js` to the `blog-price-mentions` `currentFiles` list, so the pricing-change workflow knows the review's narrative quotes green fees. Same `mmg-tools` commit as the sync |

The two `mmg-tools` rows are only checked when `../mmg-tools` is checked out (CI has no sibling), so they are enforced locally by the pre-push hook and by running the check by hand. If the check passes with the note "sibling checks skipped", you are not in a workspace that can verify them: say so.

**Verify visually after deploy** (a green check does not prove the page reads right): `/tools/green-fees` shows "View full guide" for the new course, and `/zh/tools/green-fees` links to `/zh/guides/{slug}`. On the Course Guides PWA the "Read full review" link must sit level with its neighbours in the same row (it is pinned to the card bottom with `margin-top:auto`; do not put a fixed top margin back on `.course-review-link`), and switching the language to DE must give a `/de/guides/{slug}` link.

**Looked at and deliberately not part of this:** `GUIDE_IMAGES` in `GuidesIndexView.jsx` (fallback only; `guide.img` from `liveGuides` wins), `FILE_NAME_OVERRIDES` in `export-live-guide-content.mjs` (legacy names only), `COURSE_DESTINATIONS` in `golf-courses-helpers.js` (used only by `check-course-destinations.js`; it needs an entry only for courses featured on the homepage), `prototypes/*`, `ui-preview`, and the sample-route lists in `lighthouse-scorecard.mjs` / `responsive-audit.js`. The standalone apps and the internal quote builder do not link to reviews. Searched across the whole workspace on 2026-09-26.
