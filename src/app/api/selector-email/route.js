      ...
      .map(
        c => `
          <div style="margin-bottom:20px;border-left:3px solid #B8973C;padding-left:16px;">
            <p style="margin:0 0 4px;font-family:Georgia,serif;font-size:14px;font-weight:bold;color:#2D4A3E;">${c.name}</p>
            <p style="margin:0;font-family:'Jost',Arial,sans-serif;font-size:12px;color:#666;">${c.why || 'A strong match for your game.'}</p>
          </div>
        `
      )
      .join('')
...
<body style="margin:0;padding:0;background:#F7F4EF;font-family:Georgia,serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F7F4EF;padding:48px 24px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;">
        ...
        <tr><td style="background:#2D4A3E;border-radius:4px 4px 0 0;padding:32px 40px 24px;">
          <h1 style="margin:0;font-family:Georgia,serif;font-size:24px;font-weight:400;color:#F7F4EF;line-height:1.2;">Your Course Recommendations</h1>
          <p style="margin:12px 0 0;font-family:'Jost',Arial,sans-serif;font-size:12px;color:rgba(247,244,239,0.7);">Matched to your game and preferences</p>
        </td></tr>
        <tr><td style="background:#fff;border-radius:0 0 4px 4px;padding:32px 40px 40px;">
          <p style="margin:0 0 20px;font-family:'Jost',Arial,sans-serif;font-size:13px;color:#666;line-height:1.6;">Thank you for using the course finder. Based on your skill level, budget, and location, here are the three courses that best match your trip:</p>
          ${courseListHtml}
          <hr style="border:none;border-top:1px solid #EDE9E1;margin:32px 0;">
          <p style="margin:0 0 12px;font-family:Georgia,serif;font-size:15px;line-height:1.7;color:#2C2A27;">What's next? Contact me directly and I'll handle the tee times, transfers, and any coaching you want. All your questions answered in English.</p>
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: 'Your Mallorca Golf Course Recommendations',
      html: emailHtml,
    })

    if (error) {
      console.error('[selector-email] Resend error:', error)
      return NextResponse.json({ ok: false, error: 'Failed to send email.' }, { status: 500 })
    }

    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error('[selector-email] Error:', err)
