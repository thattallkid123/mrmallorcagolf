// Localized copy for the "t-golf-palma-review" guide. See src/lib/guide-post-content-localized.js for the assembly and lookup API.
const content = {
  "es": {
    "metadata": {
      "title": "T Golf Palma - Análisis y Tarifas",
      "description": "T Golf Palma en Mallorca: hasta 150 € en temporada alta, 6.027 m, estado 9/10."
    },
    "meta": {
      "badge": "Análisis de Campo",
      "readTime": "6 min",
      "updated": "Septiembre 2026",
      "title": "T Golf Palma - Análisis y Tarifas",
      "intro": "Greenes rápidos, a menudo pequeños y con ondulaciones, y más de un hoyo de riesgo y recompensa que te hacen pensar desde el tee. Un 9 sobre 10, un campo que merece juzgarse por sí mismo y no solo por compartir nombre con su campo hermano, T Golf Calvià.",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual Golf: Análisis Honesto 2026"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner Golf: Análisis Honesto 2026"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "Los Mejores Campos de Golf en Mallorca 2026"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "¿Cuánto Cuesta el Golf en Mallorca?"
        }
      ]
    },
    "blocks": [
      {
        "alt": "El green protegido por agua en uno de los par 4 del noveno de vuelta en T Golf Palma",
        "caption": "Agua y búnkeres protegen el green en uno de los par 4 del noveno de vuelta en T Golf Palma."
      },
      {
        "text": "Tuvimos la primera salida del día en T Golf Palma, las 7:30, sin nadie por delante. Los jardineros estaban trabajando en el campo y pararon para felicitarnos por un par de golpes al pasar. Un campo tan cerca de Palma rara vez está así de tranquilo."
      },
      {
        "text": "El Entorno"
      },
      {
        "text": "T Golf Palma cambió de manos en marzo de 2022, cuando T Club compró el campo y lo rebautizó, y parte de su reputación viene de compartir nombre con T Golf Calvià. Fui pensando que eso podía estar haciendo parte del trabajo. No es así. Jack Nicklaus diseñó el campo, el único suyo en Mallorca, que abrió en 2006 antes de una renovación completa en 2022."
      },
      {
        "text": "Está apartado de casi todo lo demás en la isla. Pasan aviones desde el aeropuerto de Palma, pero durante casi toda la vuelta no se ve ningún edificio. Aislado pero bajo una ruta de vuelo es una combinación curiosa."
      },
      {
        "text": "Hoyos que conviene conocer"
      },
      {
        "text": "El hoyo 5 es el más difícil de la tarjeta, un par 4 de 364 m con un tee shot estrecho y un green angosto para terminar. No hay mucho margen de error en ningún extremo."
      },
      {
        "text": "El hoyo 8, de 334 m, es el mejor ejemplo del carácter del campo. El agua corre por todo el lado derecho y el tee shot tienta a sacar el driver. Yo jugué seguro con un hierro 4 a la izquierda, lo que aun así dejó un wedge a una franja de green muy estrecha, nada fácil de acertar. Quedarse corto no significa que el problema haya terminado."
      },
      {
        "alt": "El obstáculo de agua a la derecha del hoyo 8 en T Golf Palma bajo un cielo nublado",
        "caption": "El agua a la derecha en el 8. Incluso la ruta segura desde el tee deja un segundo golpe exigente hacia un green estrecho."
      },
      {
        "text": "El hoyo 12 mide 392 m con un green inclinado, el par 4 más difícil del noveno de vuelta sobre el papel. Juega tan difícil como sugiere la distancia."
      },
      {
        "text": "El hoyo 15 fue el más bonito del campo. El agua recorre todo el lado derecho y hay árboles a la izquierda, así que ya es estrecho antes de llegar al segundo golpe, que tiene que volver a cruzar el agua para alcanzar el green. Dejé el mío cerca de la bandera en la zona de aterrizaje estrecha e hice el birdie también, así que sí es posible."
      },
      {
        "alt": "Golfista jugando el golpe de approach sobre el agua en el hoyo 15 de T Golf Palma",
        "caption": "El approach al 15. Agua a la derecha y otro carry sobre el agua hacia el green."
      },
      {
        "text": "El hoyo 18 fue el tee shot más difícil de encarar de todo el día, jugado desde los tees traseros a 379 m. Pegué lo que creí que era un buen drive en posición A y nunca encontramos la bola. Eso me costó la opción de terminar bajo par, y cerré con un bogey para un 72."
      },
      {
        "alt": "La calle del hoyo 18 en T Golf Palma girando hacia el club house",
        "caption": "Mirando el 18. El tee shot es más difícil de encarar de lo que parece desde aquí."
      },
      {
        "text": "Estado del Campo"
      },
      {
        "text": "El estado del campo fue excelente en todo momento, a la altura de Calvià. Los greenes estaban rápidos y pequeños, y el rough agarra el palo lo suficiente como para que fallar la calle se penalice, y cerca del green un chip difícil se complica aún más por lo mucho que ondulan los greenes. Jugamos en un día blando porque habían regado mucho con los aspersores; cuando el terreno se endurezca, algunos de esos chips serán aún más difíciles."
      },
      {
        "alt": "Un avión sobrevolando la calle y los búnkeres en T Golf Palma con pinos detrás",
        "caption": "Calles tranquilas con un avión sobrevolando. El campo está apartado de casi todo lo demás en la isla, así que a pesar de la ruta de vuelo nunca se siente urbanizado."
      },
      {
        "text": "Servicio E Instalaciones"
      },
      {
        "text": "El servicio fue un punto fuerte. Alguien recorre el campo con bebidas, y el mismo empleado nos esperaba en el green del 18 para saludarnos al terminar. Llegamos antes que la mayoría del personal para la primera salida, así que no puedo hablar del green de approach, el putting green ni el campo de prácticas por experiencia propia, pero la tienda de golf estaba bien surtida. Las instalaciones de práctica según la ficha son un campo de 42 puestos con 14 cubiertos y entrenadores PGA en el club. La terraza sobre el 18 es un buen sitio para terminar, con vista clara a todo el hoyo de cierre."
      },
      {
        "alt": "La entrada y la tienda de golf en T Golf Palma al anochecer",
        "caption": "La tienda de golf en T Golf Palma, bien surtida, con la gata residente del club haciendo acto de presencia."
      },
      {
        "text": "Información Práctica"
      },
      {
        "items": [
          [
            "75 €",
            "Green fee el día que jugamos"
          ],
          [
            "9/10",
            "Valoración de Andy"
          ],
          [
            "6.027 m",
            "Tees blancos (Par 71)"
          ],
          [
            "Buggy opcional",
            "El campo es caminable, no muy montañoso"
          ]
        ]
      },
      {
        "text": "Los green fees van desde 80 € en temporada baja hasta 150 € en temporada alta en 2026, más una licencia federativa diaria obligatoria de 3 €. Nosotros pagamos alrededor de 75 € el día que jugamos. Hay buggies disponibles y yo usé uno, pero el campo es caminable y no muy montañoso si prefieres llevar los palos o un carrito."
      },
      {
        "text": "Una cosa a tener en cuenta: el rough agarra el palo lo suficiente como para que un green fallado suele significar un chip más difícil de lo esperado, y en un día más duro que el nuestro será aún más complicado."
      },
      {
        "text": "Preguntas Frecuentes"
      },
      {
        "text": "El límite de hándicap es 28 para hombres y 34 para mujeres, con certificado obligatorio al reservar. Caminar es viable aquí, un terreno ondulado más que montañoso, aunque la mayoría de los visitantes sigue usando buggy. Los jugadores individuales pueden reservar, pero normalmente se emparejan con otro grupo en temporada alta. Sopla una brisa suave durante casi toda la vuelta, nada que condicione la elección de palo como en campos más expuestos. La sorpresa es lo aislado que se siente estando tan cerca del aeropuerto de Palma: pasan aviones, pero apenas hay otra señal de nada más allá del propio campo."
      },
      {
        "text": "Veredicto"
      },
      {
        "text": "9/10. T Golf Palma, en mi opinión, consiguió parte de su reputación inicial por compartir nombre con T Golf Calvià. Jugarlo confirmó lo contrario: el estado del campo, los hoyos de riesgo y recompensa y el cierre en el 15 y el 18 lo convierten en una vuelta igual de buena por mérito propio. Le va bien a un jugador que quiera pensar el tee shot en lugar de solo pegarle fuerte, con varios hoyos donde el control de distancia importa más que la distancia."
      },
      {
        "title": "Cuatro cosas que sabría antes de reservar T Golf Palma",
        "items": [
          [
            "Mejor hora de salida",
            "Tuvimos la primera salida a las 7:30 y tuvimos el campo para nosotros solos. Si puedes conseguir una salida temprana, hazlo."
          ],
          [
            "Elección de palo",
            "El control de distancia importa más que la distancia bruta aquí. A menudo me quedaban 80-120 yardas y necesitaba mucha precisión para puntuar bien. Un jugador de hándicap medio jugará el campo más corto que yo, pero los hierros largos hacia zonas de aterrizaje pequeñas seguirán siendo exigentes."
          ],
          [
            "Donde suelen perder golpes los visitantes",
            "El tee shot del 18 parece más sencillo de lo que juega. Estudia bien la línea antes de decidirte por el driver."
          ],
          [
            "Consejo de la terraza",
            "La terraza mira directamente al 18, un buen sitio para ver terminar a un grupo mientras esperas mesa."
          ]
        ]
      },
      {
        "text": "Es una buena opción para quien juegue varias vueltas por la zona de Palma y quiera una prueba seria sin necesitar un día entero. Si estás organizando un viaje más amplio, la <a href=\"/plan-your-trip\">guía de planificación del viaje</a> explica cómo encajar campos como este junto a <a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a>."
      },
      {
        "alt": "Andy Griffiths con su compañero de juego antes de la vuelta en T Golf Palma"
      },
      {
        "text": "¿Quieres jugar T Golf Palma con un profesional PGA que conoce cada hoyo?",
        "linkLabel": "Ver la experiencia play-with-a-pro →"
      }
    ]
  },
  "de": {
    "metadata": {
      "title": "T Golf Palma - Bewertung & Gebühren",
      "description": "T Golf Palma auf Mallorca: Jack-Nicklaus-Design, 6.027 m, 9/10 Zustand."
    },
    "meta": {
      "badge": "Platz-Bewertung",
      "readTime": "6 Min.",
      "updated": "September 2026",
      "title": "T Golf Palma - Bewertung & Gebühren",
      "intro": "Schnelle Grüns, oft klein und wellig, und mehr als nur ein paar Risiko-Belohnung-Löcher, die einen vom Abschlag aus zum Nachdenken bringen. 9 von 10, und ein Platz, der für sich selbst beurteilt werden sollte, nicht nur dafür, dass er sich den Namen mit seinem Schwesterplatz T Golf Calvià teilt.",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual Golf: Ehrliche Bewertung 2026"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner Golf: Ehrliche Bewertung 2026"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "Die besten Golfplätze auf Mallorca 2026"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "Was kostet Golf auf Mallorca?"
        }
      ]
    },
    "blocks": [
      {
        "alt": "Das wassergeschützte Grün auf der Hinterneun bei T Golf Palma, von Pinien gerahmt",
        "caption": "Wasser und Bunker schützen das Grün auf einem der Par-4-Löcher der Hinterneun bei T Golf Palma."
      },
      {
        "text": "Wir hatten die erste Startzeit des Tages bei T Golf Palma, 7:30 Uhr, niemand vor uns. Die Greenkeeper arbeiteten auf dem Platz und hielten an, um ein paar Schläge zu loben, als wir vorbeikamen. Ein Platz so nah an Palma ist selten so ruhig."
      },
      {
        "text": "Die Lage"
      },
      {
        "text": "T Golf Palma hat im März 2022 den Besitzer gewechselt, als T Club den Platz kaufte und umbenannte, und einen Teil seines Rufs davon, dass er sich den Namen mit T Golf Calvià teilt. Ich bin mit der Frage hingefahren, ob das einen Teil der Arbeit übernimmt. Das ist nicht der Fall. Jack Nicklaus hat den Platz entworfen, der einzige von ihm auf Mallorca, eröffnet 2006 vor einer kompletten Renovierung 2022."
      },
      {
        "text": "Er liegt abseits von fast allem anderen auf der Insel. Flugzeuge fliegen vom Flughafen Palma aus darüber, aber auf fast der gesamten Runde ist kein Gebäude zu sehen. Isoliert, aber unter einer Einflugschneise, ist eine merkwürdige Kombination."
      },
      {
        "text": "Löcher, die man kennen sollte"
      },
      {
        "text": "Loch 5 ist das schwerste Loch auf der Karte, ein Par 4 mit 364 m, engem Abschlag und schmalem Grün zum Abschluss. An keinem Ende bleibt viel Spielraum für Fehler."
      },
      {
        "text": "Loch 8, 334 m, ist das beste Beispiel für den Charakter des Platzes. Wasser verläuft die gesamte rechte Seite entlang, und der Abschlag verleitet dazu, den Driver aufs Wasser zu nehmen. Ich spielte sicher mit einem 4er-Eisen nach links, was trotzdem ein Wedge in ein schmales Stück Grün übrig ließ, das nicht leicht zu treffen war. Kurz zu spielen heißt nicht, dass die Schwierigkeit vorbei ist."
      },
      {
        "alt": "Der Wasserhazard rechts vom 8. Loch bei T Golf Palma unter bewölktem Himmel",
        "caption": "Das Wasser rechts auf Loch 8. Selbst die sichere Route vom Abschlag verlangt einen schwierigen zweiten Schlag auf ein schmales Grün."
      },
      {
        "text": "Loch 12 misst 392 m mit einem geneigten Grün, auf dem Papier das schwerste Par 4 der Hinterneun. Es spielt genauso schwer, wie die Länge vermuten lässt."
      },
      {
        "text": "Loch 15 war das schönste Loch auf dem Platz. Wasser verläuft die gesamte rechte Seite entlang, links stehen Bäume, sodass es schon eng ist, bevor überhaupt der zweite Schlag kommt, der erneut über das Wasser tragen muss, um das Grün zu erreichen. Ich brachte meinen auf der schmalen Landezone nah an die Fahne und machte auch das Birdie, es geht also."
      },
      {
        "alt": "Golfer spielt den Annäherungsschlag über Wasser auf dem 15. Loch bei T Golf Palma",
        "caption": "Das Anspiel auf Loch 15. Wasser rechts und ein weiterer Carry aufs Grün."
      },
      {
        "text": "Loch 18 war der schwierigste Abschlag des ganzen Tages, gespielt von den hinteren Abschlägen bei 379 m. Ich schlug, was ich für einen guten Drive in Position A hielt, und wir fanden den Ball nie wieder. Das kostete mich die Chance, unter Par abzuschließen, und ich beendete die Runde mit einem Bogey für eine 72."
      },
      {
        "alt": "Das Fairway auf dem 18. Loch bei T Golf Palma, das Richtung Clubhaus schwenkt",
        "caption": "Blick auf Loch 18. Der Abschlag ist schwerer zu treffen, als er von hier aussieht."
      },
      {
        "text": "Platzzustand"
      },
      {
        "text": "Der Platzzustand war durchgehend ausgezeichnet, auf Augenhöhe mit Calvià. Die Grüns waren schnell und klein, und der Rough hält den Schläger fest genug, dass ein verfehltes Fairway bestraft wird, und in der Nähe des Grüns wird ein schwieriger Chip durch die welligen Grüns noch schwerer. Wir spielten an einem weichen Tag, weil viel beregnet worden war; wenn der Boden fester wird, werden einige dieser Chips noch schwerer."
      },
      {
        "alt": "Ein Flugzeug fliegt über das Fairway und die Bunker bei T Golf Palma, Pinien im Hintergrund",
        "caption": "Ruhige Fairways mit einem Flugzeug darüber. Der Platz liegt abseits von fast allem anderen auf der Insel, sodass er trotz der Einflugschneise nie zugebaut wirkt."
      },
      {
        "text": "Service Und Einrichtungen"
      },
      {
        "text": "Der Service war eine Stärke. Jemand fährt mit Getränken über den Platz, und derselbe Mitarbeiter wartete am Grün von Loch 18, um uns zum Abschluss zu begrüßen. Wir kamen vor den meisten Mitarbeitern zur ersten Startzeit an, daher kann ich zu Pitching Green, Putting Green oder Driving Range aus eigener Erfahrung nichts sagen, aber der Proshop war gut sortiert. Die Übungsanlagen laut Datenlage sind eine 42-Stationen-Range mit 14 überdachten Plätzen und PGA-Trainern vor Ort. Die Terrasse mit Blick auf Loch 18 ist ein guter Ort zum Abschluss, mit freier Sicht auf das Schlussloch."
      },
      {
        "alt": "Der Eingang und der Proshop bei T Golf Palma im Abendlicht",
        "caption": "Der Proshop bei T Golf Palma, gut sortiert, mit einem Auftritt der ansässigen Clubkatze."
      },
      {
        "text": "Praktische Informationen"
      },
      {
        "items": [
          [
            "75 €",
            "Greenfee an unserem Spieltag"
          ],
          [
            "9/10",
            "Andys Bewertung"
          ],
          [
            "6.027 m",
            "Weiße Abschläge (Par 71)"
          ],
          [
            "Buggy optional",
            "Platz ist begehbar, nicht besonders hügelig"
          ]
        ]
      },
      {
        "text": "Die Greenfees reichen 2026 von 80 € in der Nebensaison bis 150 € in der Hauptsaison, dazu kommt eine verpflichtende Tages-Golflizenz von 3 €. Wir zahlten an unserem Spieltag etwa 75 €. Buggys sind verfügbar und ich habe einen genommen, aber der Platz ist begehbar und nicht besonders hügelig, falls man lieber trägt oder einen Trolley zieht."
      },
      {
        "text": "Eine Sache, auf die man vorbereitet sein sollte: Der Rough hält den Schläger fest genug, dass ein verfehltes Grün meist einen schwierigeren Chip als erwartet bedeutet, und an einem festeren Tag als unserem wird es nur schwerer."
      },
      {
        "text": "Häufige Fragen"
      },
      {
        "text": "Das Handicap-Limit liegt bei 28 für Herren und 34 für Damen, mit Ausweispflicht bei der Buchung. Zu Fuß ist hier gut machbar, eher sanft hügelig als steil, auch wenn die meisten Besucher trotzdem einen Buggy nehmen. Einzelspieler können buchen, werden in der Saison aber meist mit einer anderen Gruppe zusammengelegt. Über fast die gesamte Runde weht eine leichte Brise, nichts, das die Schlägerwahl so bestimmt wie auf exponierteren Plätzen. Die Überraschung ist, wie isoliert er sich anfühlt, obwohl er so nah am Flughafen Palma liegt: Flugzeuge fliegen darüber, aber sonst gibt es kaum ein Zeichen von irgendetwas jenseits des Platzes selbst."
      },
      {
        "text": "Fazit"
      },
      {
        "text": "9/10. T Golf Palma verdankte, wie ich finde, einen Teil seines frühen Rufs dem geteilten Namen mit T Golf Calvià. Die Runde selbst bewies das Gegenteil: Der Platzzustand, die Risiko-Belohnung-Löcher und der Schluss auf 15 und 18 machen ihn aus eigener Kraft zu einer genauso guten Runde. Er passt zu einem Spieler, der vom Abschlag aus mitdenken will, statt nur fest zu schwingen, mit mehreren Löchern, bei denen Distanzkontrolle wichtiger ist als Distanz."
      },
      {
        "title": "Vier Dinge, die ich vor der Buchung von T Golf Palma wissen würde",
        "items": [
          [
            "Beste Startzeit",
            "Wir hatten die erste Startzeit um 7:30 Uhr und den Platz für uns allein. Wenn du eine frühe Startzeit bekommen kannst, nimm sie."
          ],
          [
            "Schlägerwahl",
            "Distanzkontrolle ist hier wichtiger als reine Länge. Ich hatte oft noch 80-120 Yards und brauchte echte Präzision, um zu punkten. Ein Spieler mit mittlerem Handicap wird den Platz kürzer spielen als ich, aber lange Eisen auf kleine Landezonen bleiben trotzdem anspruchsvoll."
          ],
          [
            "Wo Besucher Schläge verlieren",
            "Der Abschlag auf 18 sieht einfacher aus, als er spielt. Schau dir die Linie genau an, bevor du dich für den Driver entscheidest."
          ],
          [
            "Terrassen-Tipp",
            "Die Terrasse blickt direkt auf Loch 18, ein guter Ort, um eine Gruppe abschließen zu sehen, während man auf einen Tisch wartet."
          ]
        ]
      },
      {
        "text": "Er passt gut zu allen, die mehrere Runden im Raum Palma spielen und einen richtigen Test wollen, ohne einen ganzen Tag zu brauchen. Wenn du eine größere Reise planst, deckt der <a href=\"/plan-your-trip\">Reiseplanungs-Guide</a> ab, wie man Plätze wie diesen neben <a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a> einplant."
      },
      {
        "alt": "Andy Griffiths mit seinem Mitspieler vor der Runde bei T Golf Palma"
      },
      {
        "text": "T Golf Palma mit einem PGA-Professional spielen, der jedes Loch kennt?",
        "linkLabel": "Das PGA-Begleiterlebnis entdecken →"
      }
    ]
  },
  "fr": {
    "metadata": {
      "title": "T Golf Palma - Avis & Tarifs",
      "description": "T Golf Palma à Majorque : design Jack Nicklaus, 6 027 m, état 9/10."
    },
    "meta": {
      "badge": "Avis Parcours",
      "readTime": "6 min",
      "updated": "Septembre 2026",
      "title": "T Golf Palma - Avis & Tarifs",
      "intro": "Des greens rapides, souvent petits et ondulés, et plus d'un trou à risque-récompense qui vous font réfléchir dès le départ. Un 9 sur 10, un parcours qui mérite d'être jugé pour lui-même, pas seulement parce qu'il partage son nom avec son parcours frère, T Golf Calvià.",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual Golf : Avis Honnête 2026"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner Golf : Avis Honnête 2026"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "Les Meilleurs Parcours de Golf à Majorque 2026"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "Combien Coûte le Golf à Majorque ?"
        }
      ]
    },
    "blocks": [
      {
        "alt": "Le green protégé par l'eau sur le retour à T Golf Palma, encadré de pins",
        "caption": "L'eau et les bunkers protègent le green sur l'un des par 4 du retour à T Golf Palma."
      },
      {
        "text": "Nous avons eu le premier départ de la journée à T Golf Palma, 7h30, personne devant nous. Les greenkeepers travaillaient sur le parcours et se sont arrêtés pour féliciter quelques coups à notre passage. Un parcours aussi proche de Palma est rarement aussi calme."
      },
      {
        "text": "Le Cadre"
      },
      {
        "text": "T Golf Palma a changé de propriétaire en mars 2022, quand T Club a acheté le parcours et l'a rebaptisé, et doit une partie de sa réputation au fait de partager son nom avec T Golf Calvià. J'y suis allé en me demandant si cela faisait une partie du travail. Ce n'est pas le cas. Jack Nicklaus a dessiné le parcours, le seul des siens à Majorque, ouvert en 2006 avant une rénovation complète en 2022."
      },
      {
        "text": "Il est à l'écart de presque tout le reste de l'île. Des avions passent depuis l'aéroport de Palma, mais sur presque tout le parcours, on ne voit aucun bâtiment. Isolé mais sous un couloir aérien, c'est une combinaison curieuse."
      },
      {
        "text": "Trous à connaître"
      },
      {
        "text": "Le trou 5 est le plus difficile de la carte, un par 4 de 364 m avec un départ étroit et un green resserré pour finir. Il n'y a pas beaucoup de marge d'erreur, ni d'un côté ni de l'autre."
      },
      {
        "text": "Le trou 8, 334 m, est le meilleur exemple du caractère du parcours. L'eau longe tout le côté droit et le départ donne envie de sortir le driver. J'ai joué la sécurité avec un fer 4 à gauche, ce qui a quand même laissé un wedge vers une bande de green étroite, pas facile à toucher. Jouer court ne signifie pas que le problème est réglé."
      },
      {
        "alt": "L'obstacle d'eau à droite du trou 8 à T Golf Palma sous un ciel nuageux",
        "caption": "L'eau à droite sur le 8. Même la route sûre depuis le départ laisse un deuxième coup exigeant vers un green étroit."
      },
      {
        "text": "Le trou 12 fait 392 m avec un green en pente, le par 4 le plus difficile du retour sur le papier. Il joue aussi difficile que la distance le suggère."
      },
      {
        "text": "Le trou 15 était le plus beau du parcours. L'eau longe tout le côté droit et des arbres bordent la gauche, donc c'est déjà étroit avant même le deuxième coup, qui doit à nouveau traverser l'eau pour atteindre le green. J'ai amené le mien près du drapeau sur la zone d'arrivée étroite et j'y ai aussi fait le birdie, donc c'est possible."
      },
      {
        "alt": "Golfeur jouant le coup d'approche au-dessus de l'eau sur le trou 15 à T Golf Palma",
        "caption": "L'approche sur le 15. De l'eau à droite et un nouveau carry vers le green."
      },
      {
        "text": "Le trou 18 a été le départ le plus difficile à cadrer de toute la journée, joué depuis les départs arrière à 379 m. J'ai frappé ce que je pensais être un bon drive en position A et nous n'avons jamais retrouvé la balle. Cela m'a coûté la chance de finir sous le par, et j'ai terminé avec un bogey pour un 72."
      },
      {
        "alt": "Le fairway du trou 18 à T Golf Palma qui s'incurve vers le club house",
        "caption": "Vue sur le 18. Le départ est plus difficile à cadrer qu'il n'y paraît d'ici."
      },
      {
        "text": "État Du Parcours"
      },
      {
        "text": "L'état du parcours était excellent tout au long de la journée, au niveau de Calvià. Les greens étaient rapides et petits, et le rough retient le club assez pour qu'un fairway manqué soit pénalisé, et près du green un chip difficile devient encore plus dur à cause de l'ondulation des greens. Nous avons joué un jour souple car les arroseurs avaient beaucoup tourné ; quand le terrain durcira, certains de ces chips seront encore plus difficiles."
      },
      {
        "alt": "Un avion survolant le fairway et les bunkers à T Golf Palma, pins en arrière-plan",
        "caption": "Des fairways tranquilles avec un avion au-dessus. Le parcours est à l'écart de presque tout le reste de l'île, donc malgré le couloir aérien, il ne donne jamais une impression de surdéveloppement."
      },
      {
        "text": "Service Et Installations"
      },
      {
        "text": "Le service a été un point fort. Quelqu'un fait le tour du parcours avec des boissons, et le même membre du personnel nous attendait au green du 18 pour nous saluer à la fin. Nous sommes arrivés avant la plupart du personnel pour le premier départ, donc je ne peux pas parler du green d'approche, du putting green ni du practice par expérience personnelle, mais le pro shop était bien fourni. Les installations d'entraînement, selon la fiche, comptent un practice de 42 postes dont 14 couverts et des coachs PGA sur place. La terrasse donnant sur le 18 est un bon endroit pour finir, avec une vue dégagée sur le trou de clôture."
      },
      {
        "alt": "L'entrée et le pro shop à T Golf Palma en fin de journée",
        "caption": "Le pro shop à T Golf Palma, bien fourni, avec une apparition du chat résident du club."
      },
      {
        "text": "Informations Pratiques"
      },
      {
        "items": [
          [
            "75 €",
            "Green fee le jour où nous avons joué"
          ],
          [
            "9/10",
            "Note de Andy"
          ],
          [
            "6 027 m",
            "Départs blancs (Par 71)"
          ],
          [
            "Voiturette en option",
            "Le parcours se marche, pas très vallonné"
          ]
        ]
      },
      {
        "text": "Les green fees vont de 80 € en basse saison à 150 € en haute saison en 2026, plus une licence golf journalière obligatoire de 3 €. Nous avons payé environ 75 € le jour où nous avons joué. Des voiturettes sont disponibles et j'en ai pris une, mais le parcours se marche bien et n'est pas très vallonné si vous préférez porter votre sac ou tirer un chariot."
      },
      {
        "text": "Une chose à laquelle s'attendre : le rough retient le club assez pour qu'un green manqué signifie souvent un chip plus difficile que prévu, et par un jour plus dur que le nôtre, ce sera encore plus compliqué."
      },
      {
        "text": "Questions Fréquentes"
      },
      {
        "text": "Le handicap limite est de 28 pour les hommes et 34 pour les dames, avec certificat obligatoire à la réservation. Marcher se fait bien ici, plutôt vallonné en douceur que raide, même si la plupart des visiteurs prennent quand même une voiturette. Les joueurs seuls peuvent réserver mais sont généralement associés à un autre groupe en pleine saison. Une brise légère souffle presque tout le long du parcours, rien qui impose le choix de club comme sur des parcours plus exposés. La surprise, c'est à quel point il paraît isolé alors qu'il est si proche de l'aéroport de Palma : des avions passent, mais il n'y a presque aucun autre signe de quoi que ce soit au-delà du parcours lui-même."
      },
      {
        "text": "Verdict"
      },
      {
        "text": "9/10. T Golf Palma devait, à mon avis, une partie de sa réputation initiale au fait de partager son nom avec T Golf Calvià. La partie elle-même a prouvé le contraire : l'état du parcours, les trous de risque-récompense et la fin sur le 15 et le 18 en font une partie tout aussi bonne par ses propres mérites. Il convient à un joueur qui veut réfléchir son départ plutôt que juste frapper fort, avec plusieurs trous où le contrôle de distance compte plus que la distance."
      },
      {
        "title": "Quatre choses que je voudrais savoir avant de réserver T Golf Palma",
        "items": [
          [
            "Meilleur départ",
            "Nous avons eu le premier départ à 7h30 et le parcours pour nous seuls. Si vous pouvez obtenir un départ matinal, prenez-le."
          ],
          [
            "Choix de club",
            "Le contrôle de distance compte plus que la longueur brute ici. Il me restait souvent 80-120 yards et il fallait une vraie précision pour scorer. Un joueur au handicap moyen jouera le parcours plus court que moi, mais les longs fers vers de petites zones d'arrivée resteront exigeants."
          ],
          [
            "Où les visiteurs perdent des coups",
            "Le départ du 18 paraît plus simple qu'il ne joue. Étudiez bien la ligne avant de vous engager avec le driver."
          ],
          [
            "Conseil terrasse",
            "La terrasse donne directement sur le 18, un bon endroit pour regarder un groupe terminer en attendant une table."
          ]
        ]
      },
      {
        "text": "C'est un bon choix pour qui joue plusieurs parcours dans la région de Palma et veut un vrai test sans avoir besoin d'une journée entière. Si vous organisez un voyage plus large, le <a href=\"/plan-your-trip\">guide de planification du voyage</a> explique comment enchaîner des parcours comme celui-ci avec <a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a>."
      },
      {
        "alt": "Andy Griffiths avec son partenaire de jeu avant la partie à T Golf Palma"
      },
      {
        "text": "Envie de jouer T Golf Palma avec un professionnel PGA qui connaît chaque trou ?",
        "linkLabel": "Voir l'expérience play-with-a-pro →"
      }
    ]
  },
  "nl": {
    "metadata": {
      "title": "T Golf Palma - Review & Tarief",
      "description": "T Golf Palma op Mallorca: Jack Nicklaus-ontwerp, 6.027m, 9/10 conditie."
    },
    "meta": {
      "badge": "Baanreview",
      "readTime": "6 min",
      "updated": "September 2026",
      "title": "T Golf Palma - Review & Tarief",
      "intro": "Snelle greens, vaak klein en glooiend, en meer dan een paar risk-reward holes die je vanaf de tee aan het denken zetten. Een 9 uit 10, en een baan die op eigen merites beoordeeld moet worden, niet alleen omdat hij de naam deelt met zusterbaan T Golf Calvià.",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual Golf: Eerlijke Review 2026"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner Golf: Eerlijke Review 2026"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "Beste Golfbanen op Mallorca 2026"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "Wat kost golf op Mallorca?"
        }
      ]
    },
    "blocks": [
      {
        "alt": "De door water beschermde green op de terugweg bij T Golf Palma, omlijst door pijnbomen",
        "caption": "Water en bunkers beschermen de green op een van de par 4's op de terugweg bij T Golf Palma."
      },
      {
        "text": "Wij hadden de eerste starttijd van de dag bij T Golf Palma, 7:30 uur, niemand voor ons. De greenkeepers waren aan het werk op de baan en stopten om een paar slagen te complimenteren toen we voorbijkwamen. Een baan zo dicht bij Palma is zelden zo stil."
      },
      {
        "text": "De Ligging"
      },
      {
        "text": "T Golf Palma wisselde in maart 2022 van eigenaar, toen T Club de baan kocht en hernoemde, en dankt een deel van zijn reputatie aan het delen van de naam met T Golf Calvià. Ik ging erheen met de vraag of dat een deel van het werk deed. Dat is niet zo. Jack Nicklaus ontwierp de baan, de enige van hem op Mallorca, geopend in 2006 vóór een volledige renovatie in 2022."
      },
      {
        "text": "De baan ligt afgelegen van bijna al het andere op het eiland. Er vliegen vliegtuigen over vanaf Palma airport, maar op bijna de hele ronde is geen gebouw te zien. Afgelegen maar onder een aanvliegroute is een vreemde combinatie."
      },
      {
        "text": "Holes om te kennen"
      },
      {
        "text": "Hole 5 is de zwaarste hole op de kaart, een par 4 van 364m met een smalle tee shot en een smalle green om af te sluiten. Aan geen van beide kanten is veel ruimte voor fouten."
      },
      {
        "text": "Hole 8, 334m, is het beste voorbeeld van het karakter van de baan. Water loopt de hele rechterkant langs en de tee shot verleidt je om driver te pakken over het water. Ik speelde veilig met een 4-ijzer naar links, wat alsnog een wedge overliet naar een smalle reep green, niet makkelijk te raken. Kort spelen betekent niet dat het probleem voorbij is."
      },
      {
        "alt": "Het waterhazard rechts van hole 8 bij T Golf Palma onder een bewolkte lucht",
        "caption": "Het water rechts op hole 8. Zelfs de veilige route vanaf de tee laat een lastige tweede slag naar een smalle green over."
      },
      {
        "text": "Hole 12 meet 392m met een hellende green, op papier de zwaarste par 4 van de terugweg. Hij speelt net zo zwaar als de lengte doet vermoeden."
      },
      {
        "text": "Hole 15 was de mooiste hole op de baan. Water loopt de hele rechterkant langs en links staan bomen, dus het is al smal voordat je bij de tweede slag bent, die opnieuw over het water moet om de green te bereiken. Ik bracht de mijne dicht bij de vlag op de smalle landingszone en maakte ook de birdie, dus het kan."
      },
      {
        "alt": "Golfer speelt de approach-slag over water op hole 15 bij T Golf Palma",
        "caption": "De approach op hole 15. Water rechts en nog een carry naar de green."
      },
      {
        "text": "Hole 18 was de lastigste tee shot om te richten van de hele dag, gespeeld vanaf de achterste tees op 379m. Ik sloeg wat ik dacht dat een goede drive in positie A was en we hebben de bal nooit teruggevonden. Dat kostte me de kans om onder par te eindigen, en ik sloot af met een bogey voor een 72."
      },
      {
        "alt": "De fairway op hole 18 bij T Golf Palma die richting het clubhuis buigt",
        "caption": "Uitzicht over hole 18. De tee shot is lastiger te richten dan het van hieraf lijkt."
      },
      {
        "text": "Conditie"
      },
      {
        "text": "De conditie was doorgaans uitstekend, op hetzelfde niveau als Calvià. De greens waren snel en klein, en de rough houdt de club voldoende vast dat een gemiste fairway wordt afgestraft, en bij de green wordt een lastige chip nog lastiger door hoeveel de greens glooien. We speelden op een zachte dag omdat de sproeiers veel hadden gedraaid; als de grond harder wordt, worden sommige van die chips nog lastiger."
      },
      {
        "alt": "Een vliegtuig vliegt over de fairway en bunkers bij T Golf Palma, pijnbomen op de achtergrond",
        "caption": "Rustige fairways met een vliegtuig erboven. De baan ligt afgelegen van bijna al het andere op het eiland, dus ondanks de aanvliegroute voelt het nooit volgebouwd aan."
      },
      {
        "text": "Service En Faciliteiten"
      },
      {
        "text": "Service was een sterk punt. Iemand rijdt met drankjes over de baan, en dezelfde medewerker stond bij de green van hole 18 om ons te begroeten toen we klaar waren. We kwamen voor de eerste starttijd aan vóór de meeste medewerkers, dus ik kan uit eigen ervaring niets zeggen over de chipping green, putting green of driving range, maar de pro shop was goed gevuld. De oefenfaciliteiten volgens de gegevens zijn een range met 42 plekken waarvan 14 overdekt, met PGA-coaches aanwezig. Het terras met uitzicht op hole 18 is een goede plek om af te sluiten, met vrij zicht op de slothole."
      },
      {
        "alt": "De entree en pro shop bij T Golf Palma in de avondschemering",
        "caption": "De pro shop bij T Golf Palma, goed gevuld, met een optreden van de vaste kat van de club."
      },
      {
        "text": "Praktische Informatie"
      },
      {
        "items": [
          [
            "€75",
            "Greenfee op de dag dat we speelden"
          ],
          [
            "9/10",
            "Andy's beoordeling"
          ],
          [
            "6.027m",
            "Witte tees (Par 71)"
          ],
          [
            "Buggy optioneel",
            "Baan is te belopen, niet erg heuvelachtig"
          ]
        ]
      },
      {
        "text": "Greenfees lopen in 2026 van €80 in het laagseizoen tot €150 in het hoogseizoen, plus een verplichte dagelijkse golflicentie van €3. Wij betaalden ongeveer €75 op de dag dat we speelden. Buggy's zijn beschikbaar en ik nam er een, maar de baan is te belopen en niet erg heuvelachtig als je liever draagt of een trolley trekt."
      },
      {
        "text": "Eén ding om op te letten: de rough houdt de club voldoende vast dat een gemiste green meestal een lastigere chip betekent dan verwacht, en op een hardere dag dan wij hadden wordt dat alleen maar moeilijker."
      },
      {
        "text": "Veelgestelde Vragen"
      },
      {
        "text": "Het handicaplimiet is 28 voor heren en 34 voor dames, met certificaat verplicht bij het boeken. Lopen is hier goed te doen, licht glooiend eerder dan heuvelachtig, al nemen de meeste bezoekers toch een buggy. Singles kunnen boeken maar worden in het seizoen meestal gekoppeld aan een andere groep. Er staat vrijwel de hele ronde een lichte bries, niets dat de clubkeuze bepaalt zoals op meer blootgestelde banen. De verrassing is hoe afgelegen de baan aanvoelt, zo dicht bij Palma airport: er vliegen vliegtuigen over, maar er is verder nauwelijks een teken van iets buiten de baan zelf."
      },
      {
        "text": "Conclusie"
      },
      {
        "text": "9/10. T Golf Palma dankte, in mijn ogen, een deel van zijn vroege reputatie aan het delen van de naam met T Golf Calvià. De ronde zelf bewees het tegendeel: de conditie, de risk-reward holes en de afsluiting op 15 en 18 maken het op eigen kracht een even goede ronde. Hij past bij een speler die vanaf de tee wil nadenken in plaats van gewoon hard te slaan, met meerdere holes waar afstandscontrole belangrijker is dan afstand."
      },
      {
        "title": "Vier dingen die ik zou willen weten voor ik T Golf Palma boek",
        "items": [
          [
            "Beste starttijd",
            "Wij hadden de eerste starttijd om 7:30 uur en de baan helemaal voor onszelf. Als je een vroege starttijd kunt krijgen, doe het."
          ],
          [
            "Clubkeuze",
            "Afstandscontrole is hier belangrijker dan pure lengte. Ik had vaak nog 80-120 yards en had echte precisie nodig om te scoren. Een speler met een gemiddeld handicap speelt de baan korter dan ik, maar lange ijzers naar kleine landingszones blijven veeleisend."
          ],
          [
            "Waar bezoekers slagen verliezen",
            "De tee shot op 18 lijkt eenvoudiger dan hij speelt. Bekijk de lijn goed voordat je voor de driver kiest."
          ],
          [
            "Terrastip",
            "Het terras kijkt recht over hole 18, een goede plek om een groep te zien afsluiten terwijl je op een tafel wacht."
          ]
        ]
      },
      {
        "text": "Het is een goede optie voor wie meerdere ronden in de omgeving van Palma speelt en een serieuze test wil zonder een hele dag nodig te hebben. Ben je een grotere reis aan het plannen, dan behandelt de <a href=\"/plan-your-trip\">reisplanningsgids</a> hoe je banen als deze combineert met <a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a>."
      },
      {
        "alt": "Andy Griffiths met zijn speelpartner voor de ronde bij T Golf Palma"
      },
      {
        "text": "Wil je T Golf Palma spelen met een PGA professional die elke hole kent?",
        "linkLabel": "Bekijk de play-with-a-pro ervaring →"
      }
    ]
  },
  "sv": {
    "metadata": {
      "title": "T Golf Palma - Recension & Avgifter",
      "description": "T Golf Palma på Mallorca: Jack Nicklaus-design, 6 027 m, 9/10 skick."
    },
    "meta": {
      "badge": "Banrecension",
      "readTime": "6 min",
      "updated": "September 2026",
      "title": "T Golf Palma - Recension & Avgifter",
      "intro": "Snabba greener, ofta små och kuperade, och fler än ett par risk-reward-hål som får en att tänka från tee. En 9 av 10, och en bana som förtjänar att bedömas på egna meriter, inte bara för att den delar namn med sin systerbana T Golf Calvià.",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual Golf: Ärlig Recension 2026"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner Golf: Ärlig Recension 2026"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "Bästa Golfbanor på Mallorca 2026"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "Vad kostar golf på Mallorca?"
        }
      ]
    },
    "blocks": [
      {
        "alt": "Den vattenskyddade greenen på baksidan nio hos T Golf Palma, inramad av tallar",
        "caption": "Vatten och bunkrar skyddar greenen på ett av par 4-hålen på baksidan nio hos T Golf Palma."
      },
      {
        "text": "Vi hade dagens första starttid hos T Golf Palma, 7:30, ingen framför oss. Greenkeeperna arbetade på banan och stannade för att berömma några slag när vi gick förbi. En bana så nära Palma är sällan så här tyst."
      },
      {
        "text": "Läget"
      },
      {
        "text": "T Golf Palma bytte ägare i mars 2022, då T Club köpte banan och döpte om den, och fick en del av sitt rykte från att dela namn med T Golf Calvià. Jag åkte dit och undrade om det gjorde en del av jobbet. Det gör det inte. Jack Nicklaus designade banan, den enda av hans på Mallorca, som öppnade 2006 innan en fullständig renovering 2022."
      },
      {
        "text": "Den ligger avskild från nästan allt annat på ön. Flygplan passerar från Palma flygplats, men under nästan hela rundan syns ingen byggnad. Isolerad men under en inflygningsrutt är en märklig kombination."
      },
      {
        "text": "Hål värda att känna till"
      },
      {
        "text": "Hål 5 är det svåraste hålet på kortet, ett par 4 på 364 m med ett trångt utslag och en smal green att avsluta med. Det finns inte mycket marginal för misstag i någon ände."
      },
      {
        "text": "Hål 8, 334 m, är det bästa exemplet på banans karaktär. Vatten löper längs hela högersidan och utslaget frestar en att ta driver rakt mot det. Jag spelade säkert med en 4-järn till vänster, vilket ändå lämnade en wedge in mot en smal remsa green, inte lätt att träffa. Att lägga upp betyder inte att problemet är över."
      },
      {
        "alt": "Vattenhindret till höger om hål 8 hos T Golf Palma under en molnig himmel",
        "caption": "Vattnet till höger på hål 8. Även den säkra vägen från tee lämnar ett krävande andraslag mot en smal green."
      },
      {
        "text": "Hål 12 mäter 392 m med en sluttande green, på pappret det svåraste par 4-hålet på baksidan nio. Det spelar lika svårt som längden antyder."
      },
      {
        "text": "Hål 15 var det snyggaste hålet på banan. Vatten löper längs hela högersidan och träd kantar vänstersidan, så det är trångt redan innan andraslaget, som återigen måste ta sig över vattnet för att nå greenen. Jag fick min nära flaggan på det smala landningsområdet och gjorde birdien också, så det går."
      },
      {
        "alt": "Golfare spelar inspelet över vatten på hål 15 hos T Golf Palma",
        "caption": "Inspelet på hål 15. Vatten till höger och ännu en carry mot greenen."
      },
      {
        "text": "Hål 18 var dagens svåraste utslag att sikta, spelat från de bakre tee på 379 m. Jag slog vad jag trodde var ett bra utslag i position A och vi hittade aldrig bollen. Det kostade mig chansen att avsluta under par, och jag stängde med en bogey för en 72."
      },
      {
        "alt": "Fairwayn på hål 18 hos T Golf Palma som svänger mot klubbhuset",
        "caption": "Utsikt över hål 18. Utslaget är svårare att sikta än det ser ut härifrån."
      },
      {
        "text": "Banans Skick"
      },
      {
        "text": "Skicket var genomgående utmärkt, i klass med Calvià. Greenerna var snabba och små, och roughen håller fast klubban tillräckligt för att en missad fairway straffas, och nära greenen blir en svår chip ännu svårare av hur mycket greenerna kuperar sig. Vi spelade en mjuk dag eftersom sprinklerna hade kört mycket; när marken hårdnar blir några av de där chiparna ännu svårare."
      },
      {
        "alt": "Ett flygplan flyger över fairwayn och bunkrarna hos T Golf Palma, tallar i bakgrunden",
        "caption": "Lugna fairways med ett flygplan ovanför. Banan ligger avskild från nästan allt annat på ön, så trots inflygningsrutten känns den aldrig nedbyggd."
      },
      {
        "text": "Service Och Anläggningar"
      },
      {
        "text": "Servicen var en styrka. Någon kör runt banan med drycker, och samma medarbetare väntade vid greenen på hål 18 för att hälsa när vi var klara. Vi kom före de flesta av personalen till den första starttiden, så jag kan inte uttala mig om chipping green, puttinggreen eller driving range av egen erfarenhet, men proshopen var välfylld. Träningsanläggningarna enligt uppgift är en range med 42 platser varav 14 under tak, med PGA-tränare på plats. Terrassen med utsikt över hål 18 är ett bra ställe att avsluta på, med fri sikt över avslutningshålet."
      },
      {
        "alt": "Entrén och proshopen hos T Golf Palma i kvällsljuset",
        "caption": "Proshopen hos T Golf Palma, välfylld, med ett besök av klubbens fasta katt."
      },
      {
        "text": "Praktisk Information"
      },
      {
        "items": [
          [
            "75 €",
            "Greenfee den dag vi spelade"
          ],
          [
            "9/10",
            "Andys betyg"
          ],
          [
            "6 027 m",
            "Vita tee (Par 71)"
          ],
          [
            "Buggy valfritt",
            "Banan går att gå, inte särskilt kuperad"
          ]
        ]
      },
      {
        "text": "Greenfee ligger 2026 mellan 80 € i lågsäsong och 150 € i högsäsong, plus en obligatorisk daglig golflicens på 3 €. Vi betalade ungefär 75 € den dag vi spelade. Buggy finns tillgängligt och jag tog en, men banan går att gå och är inte särskilt kuperad om man hellre bär bagen eller drar en dragvagn."
      },
      {
        "text": "En sak att vara beredd på: roughen håller fast klubban tillräckligt för att en missad green oftast betyder en svårare chip än väntat, och på en hårdare dag än vår blir det bara tuffare."
      },
      {
        "text": "Vanliga Frågor"
      },
      {
        "text": "Handicapgränsen är 28 för herrar och 34 för damer, med krav på certifikat vid bokning. Att gå fungerar bra här, mer svagt kuperad än backig, även om de flesta besökare ändå tar buggy. Singelspelare kan boka men slås oftast ihop med en annan grupp under säsong. Det blåser en lätt bris genom nästan hela rundan, inget som styr klubbval på samma sätt som på mer exponerade banor. Överraskningen är hur isolerad den känns så nära Palma flygplats: flygplan passerar, men det finns knappt något annat tecken på något bortom banan själv."
      },
      {
        "text": "Slutsats"
      },
      {
        "text": "9/10. T Golf Palma fick, som jag ser det, en del av sitt tidiga rykte från att dela namn med T Golf Calvià. Själva rundan bevisade motsatsen: skicket, risk-reward-hålen och avslutningen på 15 och 18 gör det på egna meriter till en lika bra runda. Den passar en spelare som vill tänka igenom utslaget snarare än att bara svinga hårt, med flera hål där distanskontroll betyder mer än distans."
      },
      {
        "title": "Fyra saker jag skulle vilja veta innan jag bokar T Golf Palma",
        "items": [
          [
            "Bästa starttid",
            "Vi hade den första starttiden 7:30 och hade banan för oss själva. Om du kan få en tidig starttid, ta den."
          ],
          [
            "Klubbval",
            "Distanskontroll betyder mer än ren längd här. Jag hade ofta 80-120 yards kvar och behövde verklig precision för att göra poäng. En spelare med medelhandicap spelar banan kortare än jag gjorde, men långa järn mot små landningsytor är fortfarande krävande."
          ],
          [
            "Där besökare tappar slag",
            "Utslaget på 18 ser enklare ut än det spelar. Titta noga på linjen innan du bestämmer dig för driver."
          ],
          [
            "Terrasstips",
            "Terrassen ser rakt ner över hål 18, ett bra ställe att se en grupp avsluta medan man väntar på ett bord."
          ]
        ]
      },
      {
        "text": "Den passar bra för den som spelar flera rundor i Palma-området och vill ha ett rejält test utan att behöva en hel dag. Om du planerar en större resa täcker <a href=\"/plan-your-trip\">reseplaneringsguiden</a> hur man lägger in banor som denna tillsammans med <a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a>."
      },
      {
        "alt": "Andy Griffiths med sin spelpartner före rundan hos T Golf Palma"
      },
      {
        "text": "Vill du spela T Golf Palma med en PGA-professional som känner varje hål?",
        "linkLabel": "Se play-with-a-pro-upplevelsen →"
      }
    ]
  },
  "zh": {
    "metadata": {
      "title": "T Golf Palma 评测（2026）— 果岭费、球场状况与真实评分",
      "description": "PGA职业球员于2026年9月亲身体验后的T Golf Palma评测。旺季最高150欧元,淡季80欧元起。杰克·尼克劳斯设计,白色发球台6027米,状态评分9/10。"
    },
    "meta": {
      "badge": "球场评测",
      "readTime": "6分钟",
      "updated": "2026年9月",
      "title": "T Golf Palma 评测 — PGA职业球员的真实评价（2026）",
      "intro": "果岭又快又小,还常常有起伏,好几个洞都是风险与回报并存,逼你在发球台上多想一步。综合评分9/10,这座球场值得凭自身实力被评价,而不只是因为和姐妹球场T Golf Calvià同名。",
      "related": [
        {
          "slug": "son-gual-review",
          "title": "Son Gual高尔夫：2026年真实评测"
        },
        {
          "slug": "son-muntaner-review",
          "title": "Son Muntaner高尔夫：2026年真实评测"
        },
        {
          "slug": "best-golf-courses-mallorca",
          "title": "2026年马略卡岛最佳高尔夫球场"
        },
        {
          "slug": "golf-cost-mallorca",
          "title": "在马略卡岛打高尔夫要花多少钱？"
        }
      ]
    },
    "blocks": [
      {
        "alt": "T Golf Palma后九洞被水环绕的果岭,松树环绕",
        "caption": "水障碍和沙坑守护着T Golf Palma后九洞其中一个四杆洞的果岭。"
      },
      {
        "text": "我们打了当天T Golf Palma的第一个开球时间,早上7点30分,前面没有人。球场维护人员正在场上工作,我们经过时停下来称赞了几杆好球。这么近帕尔马的球场,很少能这么安静。"
      },
      {
        "text": "球场环境"
      },
      {
        "text": "T Golf Palma在2022年3月易主,T Club买下球场并为其改名,球场的名气也有一部分来自和T Golf Calvià同名。我出发前还在想,这个名字是不是替它省了不少力气。事实并非如此。球场由杰克·尼克劳斯设计,是他在马略卡岛唯一的作品,2006年开业,2022年经过全面翻新。"
      },
      {
        "text": "球场几乎与岛上其他一切都隔绝开来。飞机会从帕尔马机场方向飞过,但几乎整轮球都看不到一栋建筑。既孤立又处在航线之下,是个奇怪的组合。"
      },
      {
        "text": "值得了解的关键洞"
      },
      {
        "text": "第5洞是记分卡上最难的一洞,364米的四杆洞,发球狭窄,收官的果岭也窄。两端都没有太多犯错空间。"
      },
      {
        "text": "第8洞334米,最能体现这座球场的个性。水障碍贯穿整个右侧,发球台诱惑你直接冲着水面开出一号木。我选择保守打法,用4号铁打向左侧安全区,即便如此,第二杆还是要用挖起杆瞄准一条很窄的果岭。打保守球不代表麻烦就结束了。"
      },
      {
        "alt": "T Golf Palma第8洞右侧的水障碍,天空多云",
        "caption": "第8洞右侧的水。即便走保守路线开球,第二杆瞄准狭窄果岭依然不容易。"
      },
      {
        "text": "第12洞392米,果岭有坡度,纸面上是后九洞最难的四杆洞。实际难度和距离显示的一样。"
      },
      {
        "text": "第15洞是全场最好看的一洞。水障碍贯穿整个右侧,左侧有树木,第二杆还没打之前就已经很紧张,而第二杆还得再次越过水面才能上果岭。我在狭窄的落球区把球打得很靠近旗杆,也抓到了小鸟球,说明这是能做到的。"
      },
      {
        "alt": "球手在T Golf Palma第15洞越过水面打进攻杆",
        "caption": "第15洞的进攻杆。右侧是水,第二杆还要再越过一次水面上果岭。"
      },
      {
        "text": "第18洞是全天最难瞄准方向的发球台,从后发球台出发,379米。我打出了一杆自认为落在理想位置A的球,结果始终没找到。这让我失去了打出低于标准杆成绩的机会,最终以柏忌收尾,全场72杆。"
      },
      {
        "alt": "T Golf Palma第18洞球道弯向会所方向",
        "caption": "望向第18洞。发球台瞄准方向比从这里看起来更难。"
      },
      {
        "text": "球场状态"
      },
      {
        "text": "全程球场状态出色,和Calvià不相上下。果岭又快又小,长草区抓杆能力很强,打丢球道会受到惩罚,靠近果岭时,起伏的果岭又让本就困难的切杆更难。我们打球那天因为喷灌用得多,场地偏软;等地面变硬之后,有些切杆会更难。"
      },
      {
        "alt": "一架飞机飞过T Golf Palma的球道和沙坑,背景是松树",
        "caption": "安静的球道,头顶有飞机飞过。球场几乎与岛上其他一切隔绝,所以尽管处在航线之下,也从不显得拥挤。"
      },
      {
        "text": "服务与设施"
      },
      {
        "text": "服务是一大亮点。有人开车在场上巡回提供饮品,同一位工作人员在第18洞果岭等着,在我们打完时打招呼致意。因为是当天第一个开球时间,我们比大多数员工到得还早,所以练习切杆果岭、推杆果岭和练习场我没有亲身体验可以评价,但职业商店的货品很齐全。资料显示的练习设施是一座42个打位、14个有顶棚的练习场,并配有PGA教练驻场。俯瞰第18洞的露台是收尾的好地方,能清楚看到收官洞的全貌。"
      },
      {
        "alt": "T Golf Palma傍晚灯光下的入口和职业商店",
        "caption": "T Golf Palma的职业商店,货品齐全,球会常驻的猫也出来露了面。"
      },
      {
        "text": "实用信息"
      },
      {
        "items": [
          [
            "75欧元",
            "我们打球当天的果岭费"
          ],
          [
            "9/10",
            "Andy的评分"
          ],
          [
            "6027米",
            "白色发球台(标准杆71)"
          ],
          [
            "球车可选",
            "球场可步行,坡度不算大"
          ]
        ]
      },
      {
        "text": "2026年果岭费淡季80欧元起,旺季最高150欧元,另加每日3欧元的强制球场执照费。我们打球当天大约付了75欧元。球场提供球车,我们那天租了一辆,但球场可以步行,坡度不算大,如果你更愿意自己背包或拉球车也没问题。"
      },
      {
        "text": "有一点要有心理准备:长草区抓杆能力很强,打丢果岭往往意味着比预期更难的一次切杆,如果场地比我们那天更硬,难度只会更大。"
      },
      {
        "text": "常见问题"
      },
      {
        "text": "差点限制男子28、女子34,预订时需要提供差点证明。这里步行完全可行,地势平缓起伏而非陡峭,不过大多数客人还是会选择球车。单人也可以预订,但旺季通常会与其他组并组。全程大部分时间都有轻微的风,不像一些更开阔暴露的球场那样左右选杆决定。最意外的地方是,尽管离帕尔马机场这么近,球场却给人很孤立的感觉:头顶有飞机飞过,但除此之外几乎看不到球场以外的任何痕迹。"
      },
      {
        "text": "总结"
      },
      {
        "text": "9/10。在我看来,T Golf Palma早期的名气有一部分来自和T Golf Calvià同名。而这一轮球本身证明并非如此:球场状态、风险与回报并存的球洞设计,加上第15洞和第18洞的收尾,凭自身实力就让这轮球同样精彩。这座球场适合愿意在发球台多动脑筋而不是一味用力挥杆的球手,有好几个洞距离控制比距离本身更重要。"
      },
      {
        "title": "预订T Golf Palma前我会先知道的四件事",
        "items": [
          [
            "最佳开球时间",
            "我们拿到了早上7点30分的第一个开球时间,全场只有我们。如果能约到早场,一定要约。"
          ],
          [
            "选杆建议",
            "这里距离控制比单纯的击球距离更重要。我经常剩下80-120码,需要非常精准才能拿到好成绩。中差点球手打这座球场会比我打得短,但长铁杆打向狭窄落球区依然很有挑战性。"
          ],
          [
            "游客通常在哪里丢杆",
            "第18洞的发球台看起来比实际打起来简单。开球前先仔细看清瞄准线,再决定是否用一号木。"
          ],
          [
            "露台小提示",
            "露台正对第18洞,是等座位时看一组球手收尾的好地方。"
          ]
        ]
      },
      {
        "text": "如果你打算在帕尔马地区打几轮球,又想要一次真正的考验而不需要花上一整天,这座球场是不错的选择。如果你在规划更长的行程,<a href=\"/plan-your-trip\">行程规划指南</a>介绍了如何把这样的球场和<a href=\"/guides/t-golf-calvia-review\">T Golf Calvià</a>安排在一起。"
      },
      {
        "alt": "Andy Griffiths在T Golf Palma与球伴合影,赛前拍摄"
      },
      {
        "text": "想与熟悉每一个洞的PGA职业球员同打T Golf Palma？",
        "linkLabel": "了解陪打体验 →"
      }
    ]
  }
}

export default content
