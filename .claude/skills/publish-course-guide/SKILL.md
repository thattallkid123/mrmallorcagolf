...
| `mmg-tools/guide/index.html` (Course Guides PWA) | Step 6 item 3 |
| `mmg-tools/pricing/edit/confirmed/consumer-map.json` | Add `mrmallorcagolf-real/src/lib/guide-post-content-localized/{slug}.js` to the `blog-price-mentions` `currentFiles` list, so the pricing-change workflow knows the review's narrative quotes green fees. Same separate-repo commit as the PWA link |
