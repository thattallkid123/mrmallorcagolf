...
- **`check:hotel-reference` fails when the Drive `MMG_HOTEL_REFERENCE.md` goes stale** (hit twice in one week, blocking commit/push). It is a Drive file, not a repo file, so the fix is `node scripts/export-hotel-reference.mjs` and re-run; there is nothing to stage.
