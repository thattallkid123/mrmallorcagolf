...
- **Guide translation parity:** When editing guide/review block arrays, update the localized structure in the same change. English masters are `src/lib/guide-article-content.js` and `src/lib/guide-post-content.js`; localized mirrors are `src/lib/guide-article-content-localized.js` and `src/lib/guide-post-content-localized.js`. Do not rely on the runtime merge to hide missing translated blocks. If adding/removing/reordering any `blocks` item (especially `notes`, `table`, `cta`, `facts`, `pull`, `heading`, or images), run `npm run check:guide-parity` immediately, then `npm run check:content` and `npm run check:i18n-release`. Placeholder/generic localized repair text is acceptable only as a temporary deploy-safe bridge; before considering translation work finished, replace it with proper locale-specific copy for de/es/fr/nl/sv/zh.
- **Guide notes blocks:** Do not invent visitor behaviour, course comparisons, or clubhouse claims in `notes` blocks. Keep them practical and course-grounded: tee time, wind, where shots are commonly lost, and one realistic post-round tip. Approve the English notes first, then translate those same points into de/es/fr/nl/sv/zh.
