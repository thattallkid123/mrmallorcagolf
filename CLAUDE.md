...
**Python dependencies (not automatic — a migration will silently lose these):** run `python -m pip install -r requirements.txt` on any new machine or after a Python reinstall, then confirm GA4 actually works with `python ga4_analytics/ga4_report.py`. A missing package here has silently broken GA4 reporting for weeks before, logging "GA4 unavailable" where nobody was looking. `pip install` succeeding is not proof — run the report.
...
**Tooling** (lessons from a past code-quality audit: dead `lint`, duplicate redirects, a 20-script `&&` chain, no push-time validation)
...
- **Changing a shared layout class to `display: grid`/flex means checking every page that uses it.** The 9 Aug 2026 commit "Align Signature Day layout" turned `.pwap-day` into a two-column grid meant for `.pwap-day__left`/`__right` wrapper divs that the Play With A Pro markup never had, so its two real blocks were auto-placed side by side (photos squashed left, list beside them) for five weeks. Before merging such a change, `grep -rn` the class in `src/app` and confirm each user's direct children match what the CSS expects. A CSS class with zero JSX users (`.geography`, `.intro-bar` today) is the tell of a half-finished refactor.
- **`check:hotel-reference` fails when the Drive `Reference\MMG_HOTEL_REFERENCE.md` goes stale** (hit twice in one week, blocking commit/push). It is a Drive file, not a repo file, so the fix is `node scripts/export-hotel-reference.mjs` and re-run; there is nothing to stage.
- **A course's link to its review is derived, never hand-typed.** `reviewSlug` in `golf-courses-data.js` is the single registration; the green-fees tool, both course selectors and the zh selector call `getCourseReviewSlug(name)`, and `check:guide-surfaces` fails on what can't be derived (`llms.txt`, carousel cards, the `mmg-tools` sync) and on any tool course name that matches no course. Green-fees once showed "Coming soon" for a live review for three days because ten places each typed the link. Steps: `/publish-course-guide` Step 6.5.
...
| Update Google review count | Add the review's `### Name` section (with `**Status:** ✓ Public-ready (Google review)` and `**Short excerpt:**`) to Drive `Reference/MMG_TESTIMONIALS_AND_FEEDBACK.md`, then `npm run sync:social-proof` — it regenerates `src/data/site-social-proof.json`, which drives the badge on every page. Never hand-edit the count; `check:social-proof` re-derives it. Use the `/update-testimonials` skill for the full workflow. |
...
4. Confirm **both**: `gh run list --limit 1` (watch/fix if it's running or red) AND the Vercel deployment reaching READY (Vercel MCP `list_deployments` or the dashboard). A passing local `check:content` before commit does not guarantee CI passes — CI can fail on things a local run can't see (e.g. a check that only works because a file/sibling-repo happens to exist on this machine). **If `list_deployments` shows no entry at all for the pushed sha after a few minutes** (normal turnaround is ~3 seconds), don't just wait — check `gh api repos/thattallkid123/mrmallorcagolf/commits/<sha>/statuses`; an empty array despite a completed CI check-run means Vercel's GitHub webhook never fired (a GitHub-side outage can cause this — the banner only shows in the Vercel dashboard, not via API). An empty `git commit --allow-empty` + push often re-triggers it; otherwise ask Andy to check the dashboard and use "Create Deployment" manually.
...
- **Financial:** `Business Operations & Financial/MMG_INCOME_AND_EXPENSES_2026.xlsx` + `Business Operations & Financial/Extra/MMG_EXTRA_COACHING_2026.xlsx`
...
- **Pre-deploy:** required steps are in the Completion Gate above. `npm run check:visual` (Playwright) is deliberately *not* part of it, and must be run against a production build, not `next dev` — against dev, on-demand route compilation blows Playwright's 60s `page.goto` timeout on nearly every route (a dev-mode run failed/timed out on most tests and took ~8x longer than a production-build run, which finishes in a few minutes with real signal). Run it by hand when a change is visual/layout-affecting: `npm run build`, `npx next start -p <port>`, then `PLAYWRIGHT_PORT=<port> npx playwright test`.
