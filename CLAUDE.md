...
- **Knowledge skills:** `Skills/MMG_SKILL_*.md` (17 skills currently synced - frontend-design-mmg, mmg-autonomo-filing, mmg-blog-writing, mmg-business-operations, mmg-chinese-backlog, mmg-chinese-content, mmg-client-docs, mmg-content-pipeline, mmg-email-management, mmg-hermes-ops, mmg-partnerships, mmg-repurpose, mmg-seo-content, mr-mallorca-golf-carousel, nextjs-mrmallorcagolf, site-operations-mmg, social-media-mmg). Synced to Claude by `SKILLS_SYNC.ps1`. Separate from repo code-workflow skills in `.claude/skills/`.
...
- **Pre-deploy:** required steps are in the Completion Gate above. `npm run check:visual` (Playwright) is deliberately *not* part of it, and must be run against a production build, not `next dev` — against dev, on-demand route compilation blows Playwright's 60s `page.goto` timeout on nearly every route (confirmed 2026-08-15: 73/84 tests failed/timed out, 47.6 min, against dev; 6.2 min and real signal — one genuine bug caught — against `next build && next start`). Run it by hand when a change is visual/layout-affecting: `npm run build`, `npx next start -p <port>`, then `PLAYWRIGHT_PORT=<port> npx playwright test`.
